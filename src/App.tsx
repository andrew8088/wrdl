...
const useWrdl = (): [Wrdl.Game, string, boolean] => {
  ...
  const setGuess = useCallback(
    (guess) => {
      _setGuess(guess);
      setValid(
        guess.length !== game.maxWordLength || Wrdl.isValidGuess(guess, game)
      );
    },
    [game]
  );
...
function App() {
  ...
  const emptyRows = Array(game.guessesRemaining - 1)
    .fill(0)
    .map((_, i) => <Guess key={i} word={"".padStart(game.maxWordLength)} />);
